...
def check_conditions():
    ...
    return (
        cpu_percent <= SCHEDULING.min_cpu
        and ram_percent <= SCHEDULING.min_ram
        and SCHEDULING.time.start <= current_hour < SCHEDULING.time.stop
    )
...
SCHEDULING = conf.iou.scheduling
mask_bank = {}
memmap_bank = {}
file_list = []
file2class_map = {}
...
def run():
    ...
    for file1_idx, file1 in enumerate(file_list):
        ...
        if (file1_idx + 1) % 10 == 0:
            ...
            if SCHEDULING and datetime.datetime.now().hour >= SCHEDULING.time.stop:
                print("Stopping...")
                break
...
def main():
    ...
    if SCHEDULING.enabled and not check_conditions():
        start = SCHEDULING.time.start

        print(f'Waiting until {start % 12} {"AM" if start < 12 else "PM"}')
